...
SCENARIOS_TO_RUN = ["drift_ot", "catch24_drift_ot"]  # novos cenários: drift_ot e catch24_drift_ot
...
DRIFT_OT_CACHE_DIRNAME = "drift_ot_cache_tmp"
...
def suggest_detector_params_from_trial(trial, detector_name: str) -> Dict[str, object]:
    """Sugere os hiperparâmetros internos do detector dentro da otimização do RF.

    Mantém os mesmos espaços de busca informados na função anexada pelo usuário,
    mas retorna um dicionário serializável para permitir cache por configuração.
    """
    if detector_name == "ADWIN":
        return {
            "delta": trial.suggest_float("adwin_delta", 1e-8, 1e-1, log=True),
            "clock": trial.suggest_int("adwin_clock", 20, 64),
            "max_buckets": trial.suggest_int("adwin_max_buckets", 2, 20),
            "min_window_length": trial.suggest_int("adwin_min_window_length", 5, 20),
            "grace_period": trial.suggest_int("adwin_grace_period", 5, 30),
        }
    if detector_name == "PageHinkley":
        return {
            "delta": trial.suggest_float("ph_delta", 1e-8, 1e-1, log=True),
            "threshold": trial.suggest_float("ph_threshold", 1e-3, 60.0, log=True),
            "alpha": trial.suggest_float("ph_alpha", 0.90, 0.99),
            "min_instances": trial.suggest_int("ph_min_instances", 5, 20),
        }
    if detector_name == "KSWIN":
        alpha = trial.suggest_float("kswin_alpha", 1e-8, 1e-1, log=True)
        window_size = trial.suggest_int("kswin_window_size", 30, 200, step=10)
        stat_frac = trial.suggest_float("kswin_stat_frac", 0.20, 0.50)
        stat_size = int(max(5, min(window_size - 5, round(window_size * stat_frac))))
        return {"alpha": alpha, "window_size": window_size, "stat_frac": stat_frac, "stat_size": stat_size, "seed": int(semente)}
    if detector_name == "CUSUM":
        return {
            "lambda_": trial.suggest_float("cusum_threshold", 0.1, 60.0, log=True),
            "delta": trial.suggest_float("cusum_delta", 1e-8, 1e-1, log=True),
            "min_n_instances": trial.suggest_int("cusum_min_instances", 5, 30),
        }
    if detector_name == "EWMAChart":
        return {
            "lambda_": trial.suggest_float("ewma_lambda", 0.05, 0.4),
            "min_n_instances": trial.suggest_int("ewma_min_instances", 5, 30),
        }
    if detector_name == "GeometricMovingAverage":
        return {
            "lambda_": trial.suggest_float("gma_threshold", 0.1, 20.0, log=True),
            "min_n_instances": trial.suggest_int("gma_min_instances", 5, 30),
            "alpha": trial.suggest_float("gma_alpha", 0.85, 0.98),
        }
    if detector_name == "HDDMAverage":
        drift_confidence = trial.suggest_float("hddma_delta", 1e-8, 1e-1, log=True)
        return {"drift_confidence": drift_confidence, "warning_confidence": drift_confidence, "test_type": "Two-sided"}
    if detector_name == "HDDMWeighted":
        drift_confidence = trial.suggest_float("hddmw_delta", 1e-8, 1e-1, log=True)
        return {
            "drift_confidence": drift_confidence,
            "warning_confidence": drift_confidence,
            "lambda_": trial.suggest_float("hddmw_lambda", 0.01, 0.3),
            "test_type": "Two-sided",
        }
    if detector_name == "SEED":
        return {
            "delta": trial.suggest_float("seed_delta", 1e-8, 1e-1, log=True),
            "block_size": trial.suggest_int("seed_block_size", 32, 100),
            "epsilon_prime": trial.suggest_float("seed_e_prime", 0.0025, 0.01, log=True),
            "alpha": trial.suggest_float("seed_alpha", 0.4, 0.8),
            "compress_term": int(trial.suggest_int("seed_compress_term", 50, 100)),
        }
    raise ValueError(f"Detector desconhecido: {detector_name}")


def detector_params_from_trial_params(detector_name: str, params: Dict[str, object]) -> Dict[str, object]:
    """Reconstrói os parâmetros do detector a partir do JSON salvo pelo Optuna."""
    if detector_name == "ADWIN":
        return {"delta": params["adwin_delta"], "clock": params["adwin_clock"], "max_buckets": params["adwin_max_buckets"], "min_window_length": params["adwin_min_window_length"], "grace_period": params["adwin_grace_period"]}
    if detector_name == "PageHinkley":
        return {"delta": params["ph_delta"], "threshold": params["ph_threshold"], "alpha": params["ph_alpha"], "min_instances": params["ph_min_instances"]}
    if detector_name == "KSWIN":
        window_size = int(params["kswin_window_size"])
        stat_frac = float(params["kswin_stat_frac"])
        stat_size = int(max(5, min(window_size - 5, round(window_size * stat_frac))))
        return {"alpha": params["kswin_alpha"], "window_size": window_size, "stat_frac": stat_frac, "stat_size": stat_size, "seed": int(semente)}
    if detector_name == "CUSUM":
        return {"lambda_": params["cusum_threshold"], "delta": params["cusum_delta"], "min_n_instances": params["cusum_min_instances"]}
    if detector_name == "EWMAChart":
        return {"lambda_": params["ewma_lambda"], "min_n_instances": params["ewma_min_instances"]}
    if detector_name == "GeometricMovingAverage":
        return {"lambda_": params["gma_threshold"], "min_n_instances": params["gma_min_instances"], "alpha": params["gma_alpha"]}
    if detector_name == "HDDMAverage":
        drift_confidence = params["hddma_delta"]
        return {"drift_confidence": drift_confidence, "warning_confidence": drift_confidence, "test_type": "Two-sided"}
    if detector_name == "HDDMWeighted":
        drift_confidence = params["hddmw_delta"]
        return {"drift_confidence": drift_confidence, "warning_confidence": drift_confidence, "lambda_": params["hddmw_lambda"], "test_type": "Two-sided"}
    if detector_name == "SEED":
        return {"delta": params["seed_delta"], "block_size": params["seed_block_size"], "epsilon_prime": params["seed_e_prime"], "alpha": params["seed_alpha"], "compress_term": int(params["seed_compress_term"])}
    raise ValueError(f"Detector desconhecido: {detector_name}")


def build_detector_list_from_params(detector_name: str, num_features: int, params: Dict[str, object]):
    if params is None:
        return build_default_detector_list(detector_name, num_features)
    if detector_name == "ADWIN":
        return [ADWIN(delta=params["delta"], clock=int(params["clock"]), max_buckets=int(params["max_buckets"]), min_window_length=int(params["min_window_length"]), grace_period=int(params["grace_period"])) for _ in range(num_features)]
    if detector_name == "PageHinkley":
        return [PageHinkley(delta=params["delta"], threshold=params["threshold"], alpha=params["alpha"], min_instances=int(params["min_instances"])) for _ in range(num_features)]
    if detector_name == "KSWIN":
        return [KSWIN(alpha=params["alpha"], window_size=int(params["window_size"]), stat_size=int(params["stat_size"]), seed=int(params.get("seed", semente))) for _ in range(num_features)]
    if detector_name == "CUSUM":
        return [CUSUM(lambda_=params["lambda_"], delta=params["delta"], min_n_instances=int(params["min_n_instances"])) for _ in range(num_features)]
    if detector_name == "EWMAChart":
        return [EWMAChart(lambda_=params["lambda_"], min_n_instances=int(params["min_n_instances"])) for _ in range(num_features)]
    if detector_name == "GeometricMovingAverage":
        return [GeometricMovingAverage(lambda_=params["lambda_"], min_n_instances=int(params["min_n_instances"]), alpha=params["alpha"]) for _ in range(num_features)]
    if detector_name == "HDDMAverage":
        return [HDDMAverage(drift_confidence=params["drift_confidence"], warning_confidence=params["warning_confidence"], test_type=params.get("test_type", "Two-sided")) for _ in range(num_features)]
    if detector_name == "HDDMWeighted":
        return [HDDMWeighted(drift_confidence=params["drift_confidence"], warning_confidence=params["warning_confidence"], lambda_=params["lambda_"], test_type=params.get("test_type", "Two-sided")) for _ in range(num_features)]
    if detector_name == "SEED":
        return [SEED(delta=params["delta"], block_size=int(params["block_size"]), epsilon_prime=params["epsilon_prime"], alpha=params["alpha"], compress_term=int(params["compress_term"])) for _ in range(num_features)]
    raise ValueError(f"Detector desconhecido: {detector_name}")


def stable_hash_dict(d: Dict[str, object]) -> str:
    payload = json.dumps(to_python_types(d or {}), sort_keys=True, separators=(",", ":"))
...
def detectar_posicoes_drift_por_feature(np_scaled, detector_name: str, detector_params: Optional[Dict[str, object]] = None) -> pd.DataFrame:
    detectors = build_detector_list_from_params(detector_name, len(FEATURES_ORIGINAIS), detector_params)
    data_np = np_scaled
...
def get_drift_ot_cache_dir(pasta_saida: str) -> str:
    d = os.path.join(pasta_saida, DRIFT_OT_CACHE_DIRNAME)
    os.makedirs(d, exist_ok=True)
    return d


def get_drift_ot_csv_path(base_dir, detector_name, scaler_name, detector_params, id_fold, split_name, nome_arquivo):
    nome_base = os.path.splitext(nome_arquivo)[0]
    h = stable_hash_dict(detector_params)
    pasta = os.path.join(base_dir, detector_name, scaler_name, f"p_{h}", f"fold_{id_fold:02d}", split_name)
    os.makedirs(pasta, exist_ok=True)
    return os.path.join(pasta, f"{nome_base}_drifts.csv")


def process_single_base_drift_ot(
    item,
    detector_name: str,
    detector_params: Dict[str, object],
    scaler_detectors,
    scaler_name: str,
    id_fold: int,
    split_name: str,
    ...
    csv_path = get_drift_ot_csv_path(
        base_dir=base_dir,
    ...
    df_drifts = detectar_posicoes_drift_por_feature(np_scaled, detector_name, detector_params=detector_params)
...
def get_drift_enriched_df_ot(item, detector_name, detector_params, scaler_name, phi_b, id_fold, split_name, pasta_saida: str, scaler_detectors=None):
    """Gera features de drift com hiperparâmetros otimizados do detector.

    O cache é indexado por detector, normalização, fold, split, base e hash dos
    hiperparâmetros internos do detector. No cenário drift_ot ele evita recomputar
    drifts dentro do mesmo trial; no catch24_drift_ot permite pré-cálculo único
    usando os parâmetros salvos em drift_ot.
    """
    base_dir = get_drift_ot_cache_dir(pasta_saida)
    df_original = item["df"].copy()
    csv_path = get_drift_ot_csv_path(
        base_dir=base_dir,
        detector_name=detector_name,
    ...
    if not os.path.exists(csv_path):
        if scaler_detectors is None:
            raise ValueError("scaler_detectors é obrigatório quando o cache drift_ot ainda não existe.")
        process_single_base_drift_ot(
            item=item,
            detector_name=detector_name,
            detector_params=detector_params,
            scaler_detectors=scaler_detectors,
...
def precompute_fixed_drift_ot_files(dfs_info, combinacoes, pasta_saida: str, detector_name: str, detector_params: Dict[str, object], scaler_name: str):
    """Pré-calcula uma única configuração de drift_ot para todos os folds/bases."""
    print(f"[INFO] Pré-cálculo drift_ot fixo | detector={detector_name} | scaler={scaler_name}", flush=True)
    base_dir = get_drift_ot_cache_dir(pasta_saida)
    for id_fold, combinacao in enumerate(combinacoes):
        treino_info = [dfs_info[idx] for idx in combinacao["treinamento"]]
        teste_info = [dfs_info[idx] for idx in combinacao["teste"]]
        ...
        for split_name, infos in [("train", treino_info), ("test", teste_info)]:
            if Parallel is None or delayed is None or N_JOBS_DRIFT <= 1:
                for item in infos:
                    process_single_base_drift_ot(item, detector_name, detector_params, scaler_detectors, scaler_name, id_fold, split_name, base_dir)
            else:
                Parallel(n_jobs=N_JOBS_DRIFT, prefer="processes")(
                    delayed(process_single_base_drift_ot)(item, detector_name, detector_params, scaler_detectors, scaler_name, id_fold, split_name, base_dir)
                    for item in infos
                )
...
def build_dataset_for_item(
    ...
    split_name: str,
    pasta_saida: str,
    detector_params: Optional[Dict[str, object]] = None,
    scaler_detectors=None,
):
    df_base = item["df"]
    ...
    if scenario_name in {"drift", "drift_ot"}:
        if scenario_name == "drift_ot":
            df_drift = get_drift_enriched_df_ot(item, detector_name, detector_params, scaler_name, phi_b, id_fold, split_name, pasta_saida, scaler_detectors=scaler_detectors)
        else:
    ...
    if scenario_name in {"catch24", "catch24_drift", "catch24_drift_ot"}:
        ...
        if scenario_name == "catch24_drift_ot":
            df_drift = get_drift_enriched_df_ot(item, detector_name, detector_params, scaler_name, phi_b, id_fold, split_name, pasta_saida, scaler_detectors=scaler_detectors)
        else:
            df_drift = get_drift_enriched_df(item, detector_name, scaler_name, phi_b, id_fold, split_name, pasta_saida)
...
def make_objective(
    ...
    fixed_catch24_window_size: Optional[int] = None,
    fixed_detector_params: Optional[Dict[str, object]] = None,
    fixed_phi_b: Optional[float] = None,
    fixed_scaler_detectors: Optional[str] = None,
):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        elif scenario_name in {"catch24_drift", "catch24_drift_ot"}:
            if fixed_catch24_window_size is None:
                raise ValueError(f"{scenario_name} requer a melhor janela do cenário catch24.")
        ...
        elif scenario_name == "drift_ot":
            phi_b = trial.suggest_float("phi_b", 1e-6, 1e-1, log=True)
            scaler_name = trial.suggest_categorical("scaler_detectors", SCALERS_DISPONIVEIS)
            detector_params = suggest_detector_params_from_trial(trial, detector_name)
            trial.set_user_attr("detector_params", to_python_types(detector_params))
        elif scenario_name == "catch24_drift_ot":
            if fixed_detector_params is None or fixed_phi_b is None or fixed_scaler_detectors is None:
                raise ValueError("catch24_drift_ot requer parâmetros salvos do cenário drift_ot.")
            detector_params = fixed_detector_params
            phi_b = float(fixed_phi_b)
            scaler_name = str(fixed_scaler_detectors)
            trial.set_user_attr("detector_params", to_python_types(detector_params))
            trial.set_user_attr("fixed_phi_b", phi_b)
            trial.set_user_attr("fixed_scaler_detectors", scaler_name)
        ...
        for fold_id, combinacao in enumerate(combinacoes):
            ...
            scaler_detectors_fold = None
            if scenario_name in {"drift_ot", "catch24_drift_ot"}:
                treino_df_concat = pd.concat([dfs_info[idx]["df"] for idx in train_ids], ignore_index=True)
                scaler_detectors_fold = build_scaler_from_name(scaler_name)
                scaler_detectors_fold.fit(treino_df_concat[FEATURES_ORIGINAIS])
            ...
            for idx in train_ids:
                X_i, y_i = build_dataset_for_item(
                    ...
                    pasta_saida=pasta_saida,
                    detector_params=detector_params,
                    scaler_detectors=scaler_detectors_fold,
                )
...
def save_study_outputs(study, pasta_scenario: str, nome_run: str, scenario_name: str, detector_name: Optional[str]):
    ...
    resumo = {
        ...
        "detector_params": to_python_types(best_trial.user_attrs.get("detector_params")),
        "fixed_phi_b": to_python_types(best_trial.user_attrs.get("fixed_phi_b")),
        "fixed_scaler_detectors": to_python_types(best_trial.user_attrs.get("fixed_scaler_detectors")),
...
def run_single_study(
    ...
    objective = make_objective(
        ...
        fixed_detector_params=fixed_detector_params,
        fixed_phi_b=fixed_phi_b,
        fixed_scaler_detectors=fixed_scaler_detectors,
    )
    study.optimize(objective, n_trials=n_trials, show_progress_bar=True, n_jobs=N_JOBS_OPT)
    return study


def load_json_or_none(path: str) -> Optional[Dict[str, object]]:
...
def get_best_catch24_window_from_previous_result(pasta_saida: str) -> int:
    """Lê a janela ótima já salva no cenário catch24 previamente executado."""
    path = os.path.join(pasta_saida, "catch24", "resultado_otimizacao.json")
    data = load_json_or_none(path)
    if data is None:
        raise FileNotFoundError(
            f"Não encontrei o JSON do cenário catch24: {path}. "
            "Execute o cenário catch24 antes ou copie o resultado_otimizacao.json para esta pasta."
        )
    params = data.get("best_params", {})
    if "window_size" not in params:
        raise KeyError(f"O JSON do catch24 não possui best_params['window_size']: {path}")
    return int(params["window_size"])


def get_best_drift_ot_config(pasta_saida: str, detector_name: str) -> Dict[str, object]:
    """Lê phi_b, scaler e hiperparâmetros internos do detector salvos em drift_ot."""
    path = os.path.join(pasta_saida, "drift_ot", detector_name, "resultado_otimizacao.json")
    data = load_json_or_none(path)
    if data is None:
        raise FileNotFoundError(
            f"Não encontrei o JSON do cenário drift_ot para {detector_name}: {path}. "
            "Execute drift_ot antes de catch24_drift_ot."
        )
    params = data.get("best_params", {})
    if "phi_b" not in params or "scaler_detectors" not in params:
        raise KeyError(f"O JSON de drift_ot não possui phi_b/scaler_detectors em best_params: {path}")
    detector_params = data.get("detector_params")
    if detector_params is None:
        detector_params = detector_params_from_trial_params(detector_name, params)
    return {
        "phi_b": float(params["phi_b"]),
        "scaler_detectors": str(params["scaler_detectors"]),
        "detector_params": detector_params,
    }
...
def run_all_experiments():
    ...
    best_catch24_window_size = None
    if "catch24_drift_ot" in SCENARIOS_TO_RUN:
        best_catch24_window_size = get_best_catch24_window_from_previous_result(PASTA_SAIDA)
        print(f"[INFO] Janela catch24 reutilizada do JSON salvo: {best_catch24_window_size}", flush=True)
        precompute_best_catch24_window_for_all_bases(dfs_info, PASTA_SAIDA, best_catch24_window_size)
        clear_runtime_caches()

    for detector_name in DETECTORES_DISPONIVEIS:
        if "drift_ot" in SCENARIOS_TO_RUN:
            print(f"\n===== Otimizando cenário: drift_ot | detector={detector_name} =====", flush=True)
            pasta_scenario = os.path.join(PASTA_SAIDA, "drift_ot", detector_name)
            resultado_path = os.path.join(pasta_scenario, "resultado_otimizacao.json")
            if os.path.exists(resultado_path):
                print(f"[SKIP] drift_ot - {detector_name} já executado.", flush=True)
            else:
                study = run_single_study(dfs_info, combinacoes, PASTA_SAIDA, "drift_ot", detector_name, N_TRIALS, semente)
                summary_rows.append(save_study_outputs(study, pasta_scenario, run_name, "drift_ot", detector_name))
                clear_runtime_caches()
                # Remove caches de trials intermediários. O cenário catch24_drift_ot
                # recalcula apenas a configuração ótima fixa, reduzindo uso de disco.
                remove_temp_dirname(PASTA_SAIDA, DRIFT_OT_CACHE_DIRNAME)

        if "catch24_drift_ot" in SCENARIOS_TO_RUN:
            print(f"\n===== Otimizando cenário: catch24_drift_ot | detector={detector_name} =====", flush=True)
            pasta_scenario = os.path.join(PASTA_SAIDA, "catch24_drift_ot", detector_name)
            resultado_path = os.path.join(pasta_scenario, "resultado_otimizacao.json")
            if os.path.exists(resultado_path):
                print(f"[SKIP] catch24_drift_ot - {detector_name} já executado.", flush=True)
                continue

            cfg = get_best_drift_ot_config(PASTA_SAIDA, detector_name)
            precompute_fixed_drift_ot_files(
                dfs_info=dfs_info,
                combinacoes=combinacoes,
                pasta_saida=PASTA_SAIDA,
                detector_name=detector_name,
                detector_params=cfg["detector_params"],
                scaler_name=cfg["scaler_detectors"],
            )
            study = run_single_study(
                dfs_info,
                combinacoes,
                PASTA_SAIDA,
                "catch24_drift_ot",
                ...
                N_TRIALS,
                semente,
                fixed_catch24_window_size=best_catch24_window_size,
                fixed_detector_params=cfg["detector_params"],
                fixed_phi_b=cfg["phi_b"],
                fixed_scaler_detectors=cfg["scaler_detectors"],
            )
            summary_rows.append(save_study_outputs(study, pasta_scenario, run_name, "catch24_drift_ot", detector_name))
            clear_runtime_caches()

    df_summary = pd.DataFrame(summary_rows)
    df_summary.to_csv(os.path.join(PASTA_SAIDA, "resumo_geral_experimentos_ot.csv"), index=False)
    return df_summary.T
...
def remove_temp_dirname(pasta_saida: str, dirname: str):
    d = os.path.join(pasta_saida, dirname)
    if os.path.isdir(d):
        shutil.rmtree(d, ignore_errors=True)
        print(f"[OK] Cache removida: {d}", flush=True)


def remove_temp_dirs(pasta_saida: str):
    for dirname in [CATCH_CACHE_DIRNAME, DRIFT_CACHE_DIRNAME, DRIFT_OT_CACHE_DIRNAME]:
        d = os.path.join(pasta_saida, dirname)
        if os.path.isdir(d):
            shutil.rmtree(d, ignore_errors=True)
